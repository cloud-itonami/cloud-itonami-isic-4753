(ns flooringops.operation
  "OperationActor -- one coordination request = one supervised actor run,
  expressed as a langgraph-clj StateGraph. The advisor
  (FlooringRetailAdvisor) is sealed into a single node (:advise); its
  proposal is ALWAYS routed through the FlooringRetailGovernor (:govern)
  ...
  human store operator/flooring-quality coordinator. The approver resumes
  with `{:approval {:status :approved}}` (or :rejected)."
  (:require [langgraph.graph :as g]
            [langgraph.checkpoint :as cp]
            [flooringops.advisor :as advisor]
            [flooringops.governor :as governor]
            [flooringops.phase :as phase]
...
(defn build
  ...
  `flooringops.store/Store`).
  opts:
    :advisor      -- a `flooringops.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; FlooringRetailAdvisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; FlooringRetailGovernor -- independent censor (separate system than the advisor).
      (g/add-node :govern
