(ns flooringops.phase
  "Phase 0->3 staged rollout for the ISIC-4753 'Retail sale of carpets,
  ...
  operations-coordination actor.
    ...
    Phase 2  assisted-coordination  -- adds installation-operation
    ...
    Phase 3  supervised auto        -- governor-clean, high-confidence
                                       ...
                                       auto-commit. `:flag-quality-concern`
  ...
  rollout milestone still to come. Flagging a quality concern always needs
  a human to actually look at it. `flooringops.governor`'s own
  `always-escalate-ops` enforces the same invariant independently -- two
  layers, not one, agree on this."
  (:require [flooringops.governor :as governor]))
...
(def phases
  ...
  {0 {:label "read-only"              :writes #{}                                                              :auto #{}}
   ...
   2 {:label "assisted-coordination"  :writes #{:log-sales-record :schedule-installation-operation
                                                 :coordinate-supply-order}                                     :auto #{}}
   3 {:label "supervised-auto"        :writes write-ops
      :auto #{:log-sales-record :schedule-installation-operation :coordinate-supply-order}}})
...
(defn verdict->disposition
  "Map a FlooringRetailGovernor verdict to a base disposition before the
  phase gate."
  [verdict]
