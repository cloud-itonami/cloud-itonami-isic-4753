(ns flooringops.render-html
  ...
  (`flooringops.operation` -> `flooringops.governor` ->
  `flooringops.store`) through a scenario adapted from this repo's own
  `flooringops.sim` demo driver (`clojure -M:dev:run`, confirmed BEFORE
  writing this file to produce a sensible ledger against the real seeded
  store ids `store-1`..`store-3` and vendor ids `vendor-1`/`vendor-2` --
  ids that DO match `flooringops.store/demo-data`, so it was safe to
  reuse rather than author from scratch), trimmed to a representative
  subset (clean auto-commits, always-escalate ops that a human then
  approves, and five distinct HARD-hold reasons) and rendered
  ...
  (:require [jp-go-dds.skin]
            ...
            [flooringops.advisor :as advisor]
            [flooringops.store :as store]
            [flooringops.operation :as op]
            [langgraph.graph :as g]))

(def ^:private coordinator
  {:actor-id "coord-1" :actor-role :flooring-store-coordinator :phase 3})

(defn- exec! [actor tid request]
  (g/run* actor {:request request :context coordinator} {:thread-id tid}))

(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "flooring-quality-coordinator-1"}}
          {:thread-id tid :resume? true}))

(defn run-demo!
  "Runs a fresh seeded store through a scenario mixing every disposition
  this actor can reach: store-1 clears a clean sales-record log (phase-3
  auto-commit), an installation-operation schedule (auto-commit), a
  low-cost supply-order naming verified vendor-1 (auto-commit), a
  high-cost supply-order (ALWAYS escalates -- approved), and a
  quality-concern flag (ALWAYS escalates -- approved); store-99
  HARD-holds as unregistered; store-3 HARD-holds as
  registered-but-unverified; a supply-order naming unverified vendor-2
  HARD-holds; an installation proposal whose advisor injects
  `:effect :commit` HARD-holds on effect-not-propose; a sales-record
  proposal that drifts into quality-dispute-resolution-finalization
  ...
  (let [db (store/seed-db)
    ...
    (exec! actor "t2-install" {:op :schedule-installation-operation :store-id "store-1"
                               :patch {:job "living-room carpet install" :date "2026-07-24"
                                       :window "09:00-13:00"}})

    (exec! actor "t3-supply-low" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "broadloom carpet restock" :quantity 40
                                          :estimated-cost 640.0 :vendor-id "vendor-1"}})

    (exec! actor "t4-supply-high" {:op :coordinate-supply-order :store-id "store-1"
                                   :patch {:item "premium wool rug collection" :quantity 15
                                           :estimated-cost 4800.0 :vendor-id "vendor-1"}})
    (approve! actor "t4-supply-high")

    (exec! actor "t5-flag" {:op :flag-quality-concern :store-id "store-1"
                            :patch {:concern "customer reports seam separation on newly installed vinyl plank flooring within one week"
                                    :confidence 0.91}})
    ...
    (exec! actor "t8-vendor" {:op :coordinate-supply-order :store-id "store-1"
                              :patch {:item "imported laminate flooring" :quantity 30
                                      :estimated-cost 350.0 :vendor-id "vendor-2"}})

    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
                                                (-advise [_ _ req]
                                                  (assoc (advisor/infer nil req) :effect :commit)))})]
      (exec! actor-direct "t9-effect" {:op :schedule-installation-operation :store-id "store-1"
                                       :patch {:job "hallway runner install" :date "2026-07-26"}}))
...
(def ^:private action-gate-rows
  ...
  ;; (README `Ops`, `flooringops.governor`/`flooringops.phase`) --
  ...
  ["        <tr><td><code>:log-sales-record</code></td><td><span class=\"ok\">phase-3 auto-commit when clean &amp; high confidence</span></td></tr>"
   "        <tr><td><code>:schedule-installation-operation</code></td><td><span class=\"ok\">phase-3 auto-commit when clean &amp; high confidence</span></td></tr>"
   "        <tr><td><code>:coordinate-supply-order</code></td><td><span class=\"warn\">phase-3 auto when low cost + verified vendor; ALWAYS human approval when estimated-cost &gt; threshold; HARD hold if vendor unverified</span></td></tr>"
   "        <tr><td><code>:flag-quality-concern</code></td><td><span class=\"warn\">ALWAYS human approval &middot; never auto at any phase &middot; surfaces concern only, never finalizes refund/warranty/liability</span></td></tr>"])
...
(defn render
  ...
  (let [ledger (vec (store/ledger db))
    ...
    (str
     "<html><head><meta charset=\"utf-8\"><title>cloud-itonami-isic-4753 &middot; flooring-retail-coordination</title><style>"
     (jp-go-dds.skin/dds+skin)
     "</style></head><body>\n"
     "<header class=\"bar\">\n"
     "  <h1>Retail sale of carpets, rugs, wall and floor coverings (ISIC 4753) — Operator Console</h1>\n"
     ...
     "<main>\n"
     "  <section class=\"card\">\n"
     "    <h2>Flooring retail stores</h2>\n"
     "    <p class=\"muted\">Demo snapshot — build-time-generated from <code>flooringops.store</code> via <code>flooringops.render-html</code> (<code>clojure -M:dev:render-html</code>), regenerated from the real actor stack.</p>\n"
     "    <table>\n"
     ...
     "    <p class=\"muted\">Registered flooring-material counterparties. An unverified vendor is a HARD hold on <code>:coordinate-supply-order</code> (ground truth from store, never proposal self-report).</p>\n"
     "    <table>\n"
     ...
     "  </section>\n"
     "  <section class=\"card\">\n"
     "    <h2>Action gate (Flooring Retail Governor)</h2>\n"
     "    <p class=\"muted\">HARD holds cannot be overridden: store unverified (registration + retail license), vendor unverified (supply-order counterparty), effect not <code>:propose</code>, and permanently out-of-scope quality-dispute-resolution finalization (refund / warranty / liability adjudication). An unregistered or unverified store never reaches a human.</p>\n"
     "    <table>\n"
