(ns flooringops.sim
  ...
  auto-commit), then an installation-operation scheduling request and a
...
(defn- approve! [actor tid]
  (g/run* actor {:approval {:status :approved :by "flooring-quality-coordinator-1"}} {:thread-id tid :resume? true}))

(defn -main [& _]
  (let [db (store/seed-db)
        coordinator-phase-1 {:actor-id "coord-1" :actor-role :flooring-store-coordinator :phase 1}
        coordinator-phase-3 {:actor-id "coord-1" :actor-role :flooring-store-coordinator :phase 3}
        actor (op/build db)]
    ...
    (let [r (exec-op actor "t1" {:op :log-sales-record :store-id "store-1"
      ...
      (println r)
      (println "-- human flooring-store coordinator approves --")
    ...
    (println "\n== schedule-installation-operation store-1 (phase 3, clean -- auto-commits) ==")
    (println (exec-op actor "t3" {:op :schedule-installation-operation :store-id "store-1"
                                  :patch {:job "living-room carpet install" :date "2026-07-24" :window "09:00-13:00"}} coordinator-phase-3))
    ...
    (println (exec-op actor "t4" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "broadloom carpet restock" :quantity 40 :estimated-cost 640.0
    ...
    (let [r (exec-op actor "t5" {:op :coordinate-supply-order :store-id "store-1"
                                 :patch {:item "premium wool rug collection" :quantity 15 :estimated-cost 4800.0
                                         :vendor-id "vendor-1"}} coordinator-phase-3)]
      (println r)
      (println "-- human flooring-store coordinator reviews & approves --")
    ...
    (let [r (exec-op actor "t6" {:op :flag-quality-concern :store-id "store-1"
                                 :patch {:concern "customer reports seam separation on newly installed vinyl plank flooring within one week" :confidence 0.91}} coordinator-phase-3)]
      (println r)
      (println "-- human flooring-store coordinator reviews & approves --")
    ...
    (println (exec-op actor "t9" {:op :coordinate-supply-order :store-id "store-1"
                                  :patch {:item "imported laminate flooring" :quantity 30 :estimated-cost 350.0
                                          :vendor-id "vendor-2"}} coordinator-phase-3))

    (println "\n== schedule-installation-operation store-1, advisor attempts direct actuation (:effect :commit) -> HARD hold ==")
    (let [actor-direct (op/build db {:advisor (reify advisor/Advisor
                                                (-advise [_ _ req]
                                                  (assoc (advisor/infer nil req) :effect :commit)))})]
      (println (exec-op actor-direct "t10" {:op :schedule-installation-operation :store-id "store-1"
                                           :patch {:job "hallway runner install" :date "2026-07-26"}} coordinator-phase-3)))
