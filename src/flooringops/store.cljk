(ns flooringops.store
  "SSoT for the ISIC-4753 'Retail sale of carpets, rugs, wall and floor
  coverings in specialized stores' operations-COORDINATION actor, behind
  ...
  carpet/rug/flooring storefront: sales/inventory/return transaction
  logging, installation/delivery scheduling, flooring-material supply-order
  coordination with registered vendors, and quality-concern flagging
  (defective material, failed/disputed installation work). It never sets
  or authorizes a shelf/unit price, never finalizes a quality-dispute
  resolution (issuing a refund, approving/denying a warranty claim,
  determining final liability for a defective-material or bad-installation
  dispute), and never itself decides who is at fault -- see
  `flooringops.governor`'s `scope-exclusion-violations`, a HARD, permanent,
  ...
  commit or escalate -- `flooringops.governor`'s `store-unverified-
...
(defn demo-data
  ...
  []
  {:stores
   {"store-1" {:store-id "store-1" :name "Riverside Carpet & Flooring Gallery"
               :registered? true :verified? true}
    "store-2" {:store-id "store-2" :name "Summit Rug & Floor Covering Showroom"
               :registered? true :verified? true}
    "store-3" {:store-id "store-3" :name "Downtown Pop-Up Flooring Kiosk (in intake)"
               :registered? true :verified? false}}
   :vendors
   {"vendor-1" {:vendor-id "vendor-1" :name "Anatolia Carpet & Rug Wholesale"
                :registered? true :verified? true}
    "vendor-2" {:vendor-id "vendor-2" :name "Unverified Import Flooring Broker Co."
                :registered? true :verified? false}}})
