(ns flooringops.advisor-test
  "Unit tests of `flooringops.advisor` proposal generation."
...
(deftest propose-installation-operation-shape
  (testing "installation-operation proposal has correct shape"
    (let [p (adv/infer db {:op :schedule-installation-operation
                           :store-id "store-2"
                           :patch {:job "living-room carpet install" :date "2026-07-24"}})]
      (is (= :schedule-installation-operation (:op p)))
...
(deftest propose-quality-concern-shape
  (testing "quality-concern proposal always escalates"
    (let [p (adv/infer db {:op :flag-quality-concern
                           :store-id "store-1"
                           :patch {:concern "seam separation on newly installed vinyl plank flooring"}})]
