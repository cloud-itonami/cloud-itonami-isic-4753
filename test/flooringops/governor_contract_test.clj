(ns flooringops.governor-contract-test
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2"
                               {:op :flag-quality-concern :store-id "store-1"
                                :patch {:concern "seam separation on new vinyl plank flooring" :confidence 0.99}}
                               ctx)]
      (is (some? result))
    ...
    (let [db (store/seed-db)
          ...
          result (exec-request actor "t2b"
                               {:op :coordinate-supply-order :store-id "store-1"
                                :patch {:item "premium wool rug collection" :quantity 1 :estimated-cost 3200.0
                                        :vendor-id "vendor-1"}}
                               ctx)]
      (is (some? result))
      (is (= 0 (count (store/coordination-log db)))
...
(deftest scope-excluded-content-hard-hold
  (testing "proposal drifting into quality-dispute-resolution-finalization scope -> permanent hard hold"
    (let [db (store/seed-db)
          actor (op/build db)
