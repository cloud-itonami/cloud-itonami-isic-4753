(ns flooringops.governor-test
  "Pure unit tests of `flooringops.governor/check` against hand-built
  proposals -- the fast, focused complement to `governor-contract-test`'s
  full-graph integration coverage."
  (:require [clojure.test :refer [deftest is testing]]
            [flooringops.advisor :as adv]
            [flooringops.governor :as gov]
            [flooringops.store :as store]))

(def store-1 {:store-id "store-1" :name "Riverside Carpet & Flooring Gallery" :registered? true :verified? true})
(def store-3 {:store-id "store-3" :name "Downtown Pop-Up Flooring Kiosk" :registered? true :verified? false})
(def vendor-1 {:vendor-id "vendor-1" :name "Anatolia Carpet & Rug Wholesale" :registered? true :verified? true})
(def vendor-2 {:vendor-id "vendor-2" :name "Unverified Import Flooring Broker Co." :registered? true :verified? false})

(defn- clean-proposal [op store-id]
  {:op op :store-id store-id :summary "s" :rationale "routine flooring store coordination"
...
(deftest vendor-check-is-scoped-to-supply-order-only
  (testing "non-supply-order ops never trip :vendor-unverified, even with no vendors registered at all"
    (let [s (store/mem-store {"store-1" store-1})]
      (doseq [op [:log-sales-record :schedule-installation-operation :flag-quality-concern]]
...
(deftest effect-not-propose-is-hard
  (testing "any :effect other than :propose is a HARD, un-overridable block"
    (let [s (store/mem-store {"store-1" store-1})
          verdict (gov/check {} nil (assoc (clean-proposal :schedule-installation-operation "store-1") :effect :commit) s)]
      (is (true? (:hard? verdict)))
...
(deftest op-outside-allowlist-is-hard
  (testing "an op outside the closed four-op allowlist is a scope violation"
    (let [s (store/mem-store {"store-1" store-1})
          verdict (gov/check {} nil (clean-proposal :finalize-quality-dispute-resolution "store-1") s)]
      (is (true? (:hard? verdict)))
      (is (some #{:op-not-allowed} (map :rule (:violations verdict)))))))

(deftest refund-finalization-content-is-hard-and-permanent
  (testing "a proposal whose rationale touches directly issuing a refund resolution is HARD-blocked regardless of op/confidence"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "issued a refund resolution to the customer without further review"
                          :confidence 0.99)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest warranty-approval-finalization-content-is-hard
  (testing "a proposal touching approving a warranty claim resolution is HARD-blocked, same as refund finalization"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :log-sales-record "store-1")
                          :rationale "approved the warranty claim resolution before the coordinator reviewed it"
                          :confidence 0.90)
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest liability-determination-content-is-hard
  (testing "a proposal touching determining final liability is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          poisoned (assoc (clean-proposal :schedule-installation-operation "store-1")
                          :summary "installer should determine final liability for the seam defect at the door")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest dispute-settlement-content-is-hard
  (testing "a proposal touching settling the quality dispute is HARD-blocked"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})
          poisoned (assoc (clean-supply-order "store-1" "vendor-1" 100.0)
                          :summary "settled the quality dispute with the customer at the loading dock")
          verdict (gov/check {} nil poisoned s)]
      (is (true? (:hard? verdict)))
      (is (some #{:scope-excluded} (map :rule (:violations verdict)))))))

(deftest legitimate-quality-concern-is-not-scope-excluded
  (testing "flagging observed defective-material/installation-dispute concerns as a QUALITY CONCERN (not a resolution finalization) never trips scope-exclusion -- this actor's core valid use case must not be self-blocked"
    (let [s (store/mem-store {"store-1" store-1})
          concern (assoc (clean-proposal :flag-quality-concern "store-1")
                         :value {:concern "customer reports seam separation on newly installed vinyl plank flooring"})
          verdict (gov/check {} nil concern s)]
      (is (empty? (filter #(= :scope-excluded (:rule %)) (:violations verdict)))
          "raw observation content (defective material/installation dispute) is exactly what this op exists to surface"))))
...
(deftest low-cost-supply-order-does-not-force-escalate
  (testing "a :coordinate-supply-order at or below the cost threshold does not trip the high-cost escalate gate"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})
          cheap (assoc (clean-supply-order "store-1" "vendor-1" 640.0) :confidence 0.9)
...
;; "refund" or "warranty"), which then accidentally matches inside the
...
(deftest default-mock-advisor-proposals-never-self-trip-scope-exclusion
  (testing "the default mock advisor's own proposals for every allowed op never trip the governor's scope-exclusion check"
    (let [s (store/mem-store {"store-1" store-1} {"vendor-1" vendor-1})]
      (doseq [op [:log-sales-record :schedule-installation-operation :coordinate-supply-order
                  :flag-quality-concern]]
        (let [patch (if (= op :coordinate-supply-order)
                      {:item "broadloom carpet restock" :estimated-cost 640.0 :vendor-id "vendor-1"}
                      {})
